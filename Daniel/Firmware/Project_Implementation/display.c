...
void seperate_and_load_characters(uint16_t number, uint8_t decimal_pos){
	...
	//uint8_t seg4Number = (number - seg1Number - (seg2Number * 10) - (seg3Number * 100)) / 1000;





	disp_characters[0] =  seg_pattern[10 + displayMode];
	//loading calculation into array
	disp_characters[1] = seg_pattern[seg1Number];
	disp_characters[2] = seg_pattern[seg2Number];
	disp_characters[3] = seg_pattern[seg3Number];
	//disp_characters[3] = seg_pattern[seg4Number];
